...
(defvar *game-scene-tower-attack-sfx*)
  ...
  (let ((main-enemy (game-scene-tower-target tower))
        ...
        (damage (* (game-scene-tower-attack-power tower)
                   (etypecase (game-scene-tower-attack-rate tower)
                     ...
                     (single-float 1.0))))
        (sfx *game-scene-tower-attack-sfx*))
    (flet ((damage (target)
             (funcall sfx)
...
(defun game-scene-tower-try-attack (tower &optional enemies)
  (let ((sfx (ecase (game-scene-tower-type tower)
               ((:square-1 :square-2)
                (curry #'play-sfx (game-scene-audio-hit-1 *game-scene-audio*)))
               ((:round-1 :round-2)
                (let ((sfx (game-scene-audio-hit-2 *game-scene-audio*))
                      (sfx-playing-p nil))
                  (eon:game-loop-once-only ()
                    (eon:add-game-loop-hook
                     (lambda ()
                       (when (and (eon:audio-playing-p sfx) (not sfx-playing-p))
                         (eon:stop-audio sfx)))
                     :after nil))
                  (lambda ()
                    (eon:game-loop-once-only ()
                      (unless (eon:audio-playing-p sfx)
                        (eon:play-audio sfx))
                      (setf sfx-playing-p t)))))
               ((nil)))))
    (flet ((attack ()
             (when (loop :for enemy :in (if (game-scene-tower-target tower) (cons (game-scene-tower-target tower) enemies) enemies)
               ...
               (let ((*game-scene-tower-attack-sfx* sfx))
